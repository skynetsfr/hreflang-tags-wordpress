<?php
/**
 * @package Hreflang Tags WP\Admin\Validator
 * @since      1.2.0
 */
...
class HREFLANG_PRO_Validator_Table extends WP_List_Table {
	...
	protected $settings = array(
		'singular' => 'hreflang_pro_validator',
		'plural'   => 'hreflang_pro_validator',
		'ajax'     => true,
	);
	...
	function __construct() {
		...
		$this->nonce    = wp_create_nonce( 'validator-table' );
	...
	private function verify_nonce() {
		if ( $this->should_verify_nonce() && ! wp_verify_nonce( filter_input( INPUT_GET, 'nonce' ), 'validator-table' ) ) {
	...
	public function show_page() {
		...
		$this->show_validation_guide();
		$this->views();
		$this->display();
	}

	public function show_validation_guide() {
		?>
			  <div class="href-content-wrapper" id="master-template">
              <h2><span><?php _e('Validation Guide','hreflang-tags-pro'); ?></span></h2>
              <h4><?php _e('Error Messages Legend','hreflang-tags-pro'); ?></h4>
              <ol>
              	<li id="errordom"><?php _e('The required DOMDocument class is not available on this server.','hreflang-tags-pro'); ?></li>
              	<p><strong><?php _e('Problem: Unfortunately, your server does not have the correct PHP modules to take advantage of our validation tool.','hreflang-tags-pro'); ?></strong></p>
              	<p><i><?php _e('Possible Solution: Contact your hosting company and ask them to compile PHP with the DOM extension enabled.','hreflang-tags-pro'); ?></i></p>
              	<li id="error404">Missing. Header response 404.</li>
              	<p><strong><?php _e('Problem: The Alternative URL cannot be found as indicated.','hreflang-tags-pro'); ?></strong></p>
              	<p><i><?php _e('Possible Solution: Please correct any typos and double check that your alternative url exists and is not set to Draft or Private.','hreflang-tags-pro'); ?></i></p>
              	<li id="error301">Missing. Header response 301.</li>
              	<p><strong><?php _e('Problem: The Alternative URL entered is no the final destination of the page. It is redirecting to another page.','hreflang-tags-pro'); ?></strong></p>
              	<p><i><?php _e('Possible Solution: Please correct any typos and double check that your alternative is not set to redirect. Even a redirect of non-www to www can cause this error to appear.','hreflang-tags-pro'); ?></i></p>
              	<li id="errornoreturn">No return tags found.</li>
              	<p><strong><?php _e('Problem: This happens when a page includes a hreflang link to an alternate language, but the linked page doesn\'t link back to it.','hreflang-tags-pro'); ?></strong><br><?php _e('This <a target="_blank" href="https://webmasters.googleblog.com/2014/07/troubleshooting-hreflang-annotations-in.html">Official Google Webmaster Central Blog</a> post explains that:<br><code>Annotations must be confirmed from the pages they are pointing to. If page A links to page B, page B must link back to page A, otherwise the annotations may not be interpreted correctly.','hreflang-tags-pro'); ?></code></p>
              	<p><i><?php _e('Possible Solution: So if you have this error reported, you should edit the Alternative URL entry to ensure there\'s a hreflang link back to the matching originating URL.','hreflang-tags-pro'); ?></i></p>
              	<li id="errornohreftarget">No hreflang tags found on target page.</li>
	...
	function display_tablenav( $which ) {
		...
		<div class="tablenav <?php echo esc_attr( $which ); ?>">
			...
			<form id="posts-filter" action="" method="get">
				...
				<input type="hidden" name="page" value="hreflang_pro_validator"/>
	...
	function get_views() {
		...
		$status_links['all'] = '<a href="' . esc_url( admin_url( 'admin.php?page=hreflang_pro_validator' . $this->page_url ) ) . '"' . $class . '>' . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts', 'hreflang-tags-pro' ), number_format_i18n( $total_posts ) ) . '</a>';
		...
		if ( is_array( $post_stati ) && $post_stati !== array() ) {
			foreach ( $post_stati as $status ) {
				...
				$status_links[ $status_name ] = '<a href="' . esc_url( add_query_arg( array( 'post_status' => $status_name ), admin_url( 'admin.php?page=hreflang_pro_validator' . $this->page_url ) ) ) . '"' . $class . '>' . sprintf( translate_nooped_plural( $status->label_count, $total ), number_format_i18n( $total ) ) . '</a>';
			}
		}
		...
		$status_links['trash'] = '<a href="' . esc_url( admin_url( 'admin.php?page=hreflang_pro_validator&post_status=trash' . $this->page_url ) ) . '"' . $class . '>' . sprintf( _nx( 'Trash <span class="count">(%s)</span>', 'Trash <span class="count">(%s)</span>', $trashed_posts, 'posts', 'hreflang-tags-pro' ), number_format_i18n( $trashed_posts ) ) . '</a>';
	...
	protected function set_pagination( $total_items ) {
		...
		$per_page = $this->get_items_per_page( 'hreflang_tags_pro_validator_posts_per_page', 20 );
	...
	function display_rows() {
		...
		if ( ( is_array( $records ) && $records !== array() ) && ( is_array( $columns ) && $columns !== array() ) ) {
			...
			foreach ( $records as $rec ) {
				$post_name = ltrim(preg_replace('/[0-9]+/', '',$rec->post_name),'-');
				...
				<?php
				$post_meta = get_post_meta($rec->ID);
				$n = 0;
				$data = array();
				$atleastone = false;
				foreach($post_meta as $key=>$value ) {
					if (stristr($key,'hreflang')) {
						...
						if (!class_exists('DOMDocument')) {
							$result[$url] = 'fail';
							$validate[$url] = '<span class="dashicons dashicons-warning" style="color:red"></span> Fail';
							$message[$url] = 'The required DOMDocument class is not available on this server. <a href="#errordom">More Info</a>';
						}
						else {
							...
							if (strpos($http_response_header[0], '404')) {
							    $result[$url] = 'success';
								$validate[$url] = '<span class="dashicons dashicons-warning" style="color:red"></span> Fail';
								$message[$url] = 'Missing. Header response 404. <a href="#error404">[More Info]</a>';
							}
							else if (strpos($http_response_header[0], '301')) {
							    $result[$url] = 'success';
								$validate[$url] = '<span class="dashicons dashicons-warning" style="color:red"></span> Fail';
								$message[$url] = 'Missing. Header response 301. <a href="#error301">[More Info]</a>';
							}
							else {
								...
								if ($hrefs->length > 0) {
									...
									if (!in_array(get_permalink($rec->ID), $urls)) {
										$result[$url] = 'success';
										$validate[$url] = '<span class="dashicons dashicons-warning" style="color:red"></span> Fail';
										$message[$url] = 'No return tags found. <a href="#errornoreturn">[More Info]</a> <a target="_blank" href="https://webmasters.googleblog.com/2014/07/troubleshooting-hreflang-annotations-in.html">How to fix</a>';
									}
									else {
										$result[$url] = 'success';
										$validate[$url] = '<span class="dashicons dashicons-yes" style="color:green"></span> Pass';
										$message[$url] = 'Your HREFLANG Tags are properly implemented.';
									}
								}
								else {
									$result[$url] = 'success';
									$validate[$url] = '<span class="dashicons dashicons-warning" style="color:red"></span> Fail';
									$message[$url] = 'No hreflang tags found on target page. <a href="#errornohreftarget">[More Info]</a> <a target="_blank" href="https://webmasters.googleblog.com/2014/07/troubleshooting-hreflang-annotations-in.html">How to fix</a>';
								}
							}
						}
					}
			    ...
			    <table class="widefat striped">
			    	<thead>
			    		<tr>
			    			<th width="45%"><?php _e('Alternative URL','hreflang-tags-pro'); ?></th>
			    			<th width="10%"><?php _e('Language','hreflang-tags-pro'); ?></th>
			    			<th width="45%"><?php _e('Validation Results','hreflang-tags-pro'); ?></th>
			    		</tr>
			    	</thead>
			    	<tbody>
			    <?php
			    ...
			    ?>
	    		<tr>
	    			<td width="45%"><?php echo $values[$n][0]; ?></td>
	    			<td width="10%"><?php echo str_replace("_", "-", $href_lang); ?></td>
	    			<td width="45%"><?php echo $validate[$values[$n][0]].': '.$message[$values[$n][0]]; ?></td>
	    		</tr>
              <?php
				$n++;
				}
	...
	protected function parse_column( $column_name, $rec ) {
		...
		switch ( $column_name ) {
			...
			/*case 'col_row_action':
				$column_value = sprintf( '<a href="javascript:void(0);" class="hreflang-validate" data-id="%1$s">Validate</a>', $rec->ID );
				break;*/
		}
